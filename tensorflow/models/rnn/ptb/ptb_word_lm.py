...
flags.DEFINE_string("model_path", None, "Path to model to load")
...
def load_model(session, model_config, train_dir, use_log_probs=False):
  ...
  if FLAGS.model_path is not None:
    logging.info("Reading model parameters from %s" % FLAGS.model_path)
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session(config=tf.ConfigProto(allow_soft_placement=allow_soft_placement, log_device_placement=log_device_placement)) \
    ...
    if FLAGS.score:
      ...
      model, _ = load_model(session, FLAGS.config_file, train_dir, use_log_probs)
      ...
      for i in range(config.max_max_epoch):
        ...
        if valid_perplexity < best_valid:
          ...
          logging.info("Model achieves new best dev perplexity %.3f - saving to path=%s".format(
            valid_perplexity, checkpoint_path))
      ...
      final_checkpoint_path = os.path.join(FLAGS.train_dir, "rnn.final.ckpt")

      logging.info("Test Perplexity: %.3f" % test_perplexity)

      logging.info("Save final model to path=%s" % final_checkpoint_path)
      model.saver.save(session, final_checkpoint_path, global_step=model.global_step)
